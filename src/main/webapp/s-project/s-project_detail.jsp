...
<%
			...
			<input type="button" value="쪽지 보내기" value="메세지" onclick="window.open('../message/message.jsp?num=<%=dto.getNum()%>'"/><br/>
		<%} 
	}%>	
	...
	<table border="1" align="center">
			<tr>
				<td align="left" colspan="3" width="580px" style="font-size:15px">
					<h3>▶ 프로젝트 관련 문의 - [작성된 댓글 수: <%=comment_count %>]</h3> 프로젝트에 대한 문의사항을 남겨주세요.
				</td>
			</tr>	
	</table>
...
<%
			 int wid=0; 
			 if(cdto.getComment_level()>0){ // 답글 일 시, 조건문 수행
				wid=10*(cdto.getComment_level()); // 답글 들여쓰기 사이즈
%>
...
<%
	...
	<script>
		function comment_removeChek(){ // 댓글 삭제 여부 JS
